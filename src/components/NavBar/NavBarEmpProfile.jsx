...
export default function NavBarEmpProfile() {
    ...
    <nav className="bg-white/70 dark:bg-gray-900/70 backdrop-blur-md border-b border-gray-200 dark:border-gray-700 fixed top-0 left-0 w-full z-50">
      <div className="max-w-screen-xl mx-auto p-4 flex justify-between items-center dark:bg-boxdark-2 dark:text-bodydark">
        ...
        <div className="hidden md:flex space-x-8 rtl:space-x-reverse items-center">
          {["Home","Profile", "Settings", "Tasks", "Tables"].map((item) => {
            let icon;
            ...
            switch (item) {
              case "Home":
                ...
                break;
              case "Tasks":
                linkPath = "/emp/tasktocomplete";
                // icon = "";
                break;
              case "Profile":
                linkPath = "/emp/profile";
                // icon = "";
                break;  
              case "Settings":
                linkPath = "/emp/settings";
                // icon = "";
                break;
              case "Tables":
                  linkPath = "/emp/tables";
        ...
        <div className="relative hidden md:flex items-center group">
          <button
            type="button"
            className="flex items-center bg-gray-200 p-2 rounded-full focus:ring-4 focus:ring-gray-300 dark:focus:ring-gray-600"
            id="user-menu-button"
          >
            {user?.img ? (
          ...
          <div className="absolute right-0 mt-46 z-50 hidden bg-white dark:bg-gray-700 rounded-lg shadow-md w-48 group-hover:block transition-all duration-200 ease-in-out">
